...
import files, php, front, mail, sql, python, node
...
def create_asso(login_asso):
  login_president = execute(sql.get_asso_president,login_asso)[env.roledefs['sql'][0]]
  ...
  execute(php.add_php,login_asso)
  execute(front.add_front,login_asso)
  execute(sql.add_sql,login_asso, mdp2)
  execute(python.add_user,login_asso)
  execute(node.add_user,login_asso)
...
def reload_generate_vhost():
  execute(front.generate_vhost_front)
  execute(php.generate_vhost_php)

@task
@runs_once
def change_for_python(login_asso):
  execute(front.change_for_python,login_asso)
  execute(php.change_for_python,login_asso)

@task
@runs_once
def change_for_php(login_asso, php_version):
  execute(front.change_for_php,login_asso)
  execute(php.change_for_php,login_asso,php_version)
...
def delete_asso(login_asso):
  ...
  execute(php.del_php,login_asso)
  execute(front.del_front,login_asso)
  execute(sql.del_sql,login_asso)
  execute(python.del_user,login_asso)
  execute(node.del_user,login_asso)
